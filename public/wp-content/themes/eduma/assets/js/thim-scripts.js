...
var thim_eduma={el_thim_pop_up_login:null,el_loginpopopform:null,el_registerPopupForm:null,el_form_purchase_course:null,ready:function(){this.getElements()
...
this.thim_single_image_popup()},getElements:function(){this.el_thim_pop_up_login=$('#thim-popup-login')
this.el_form_purchase_course=$('form[name=purchase-course]')},load:function(){this.thim_menu()
this.thim_carousel()
this.thim_contentslider()
this.counter_box()},resize:function(){},validate_form:function(form){var valid=true,email_valid=/[A-Z0-9._%+-]+@[A-Z0-9.-]+.[A-Z]{2,4}/igm
...
return false}
var $elem=teduma.el_thim_pop_up_login.find('.thim-login-container')
$elem.addClass('loading')})},add_params_purchase_course_to_el:function(el){var teduma=this
if(teduma.el_form_purchase_course.length){el.append('<p class="params-purchase-code"></p>')
var el_paramsPurchaseCode=el.find('.params-purchase-code')
$.each(teduma.el_form_purchase_course.find('input'),function(i){var inputName=$(this).attr('name')
...
if(inputName=='add-to-cart'){var urlHandleAjax=window.location.href.addQueryVar('r',Math.random())
var el_urlHandleAjax='<input name="urlHandleAjax" value="'+urlHandleAjax+'" type="hidden">'
el_paramsPurchaseCode.append(el_urlHandleAjax)}}})}},form_submission_validate:function(){$('.form-submission-login form[name=loginform]').on('submit',function(e){if(!thim_eduma.validate_form($(this))){e.preventDefault()
return false}})
...
if(header.hasClass('header_overlay')){top_site_main.css({'padding-top':height_sticky_header+'px'})
$(window).resize(function(){let height_sticky_header=header.outerHeight(true)
top_site_main.css({'padding-top':height_sticky_header+'px'})})}else{content_pusher.css({'padding-top':height_sticky_header+'px'})
$(window).resize(function(){let height_sticky_header=header.outerHeight(true)
...
$submit_wrapper.on('click',function(){$(this).closest('form').submit()})},thim_course_filter:function(){let $body=$('body')
if(!$body.hasClass('learnpress')||!$body.hasClass('archive')){return}
let ajaxCall=function(data){return $.ajax({url:$('#lp-archive-courses').data('allCoursesUrl'),type:'POST',data:data,dataType:'html',beforeSend:function(){$('#thim-course-archive').addClass('loading')},}).fail(function(){$('#thim-course-archive').removeClass('loading')}).done(function(data){let $document=$($.parseHTML(data))
...
let sendData={s:'',ref:'course',post_type:'lp_course',course_orderby:'newly-published',course_paged:1,}
$(document).on('change','.thim-course-order > select',function(){sendData.s=$('.courses-searching .course-search-filter').val()
sendData.course_orderby=$(this).val()
sendData.course_paged=1
ajaxCall(sendData)})
$(document).on('click','#lp-archive-courses > .learn-press-pagination a.page-numbers',function(e){e.preventDefault()
$('html, body').animate({'scrollTop':$('.site-content').offset().top-140,},1000)
let pageNum=parseInt($(this).text()),paged=pageNum?pageNum:1,cateArr=[],instructorArr=[],cpage=$('.learn-press-pagination.navigation.pagination ul.page-numbers li span.page-numbers.current').text(),isNext=$(this).hasClass('next')&&$(this).hasClass('page-numbers'),isPrev=$(this).hasClass('prev')&&$(this).hasClass('page-numbers')
if(!pageNum){if(isNext){paged=parseInt(cpage)+1}
if(isPrev){paged=parseInt(cpage)-1}}
$('form.thim-course-filter').find('input.filtered').each(function(){switch($(this).attr('name')){case 'course-cate-filter':cateArr.push($(this).val())
break
case 'course-instructor-filter':instructorArr.push($(this).val())
break
case 'course-price-filter':sendData.course_price_filter=$(this).val()
break
default:break}})
if($body.hasClass('category')&&$('.list-cate-filter').length<=0){let bodyClass=$body.attr('class'),cateClass=bodyClass.match(/category\-\d+/gi)[0],cateID=cateClass.split('-').pop()
cateArr.push(cateID)}
...
ajaxCall(sendData)})
$('form.thim-course-filter').on('submit',function(e){e.preventDefault()
let formData=$(this).serializeArray(),cateArr=[],instructorArr=[]
if(!formData.length){return}
$('html, body').animate({'scrollTop':$('.site-content').offset().top-140,},1000)
$(this).find('input').each(function(){let form_input=$(this)
form_input.removeClass('filtered')
if(form_input.is(':checked')){form_input.addClass('filtered')}})
$.each(formData,function(index,filter){switch(filter.name){case 'course-cate-filter':cateArr.push(filter.value)
break
case 'course-instructor-filter':instructorArr.push(filter.value)
break
case 'course-price-filter':sendData.course_price_filter=filter.value
break
default:break}})
if($body.hasClass('category')&&$('.list-cate-filter').length<=0){let bodyClass=$body.attr('class'),cateClass=bodyClass.match(/category\-\d+/gi)[0],cateID=cateClass.split('-').pop()
cateArr.push(cateID)}
...
sendData.course_instructor_filter=instructorArr
sendData.course_paged=1
ajaxCall(sendData)})},mobile_menu_toggle:function(){$(document).on('click','.menu-mobile-effect',function(e){e.stopPropagation()
$('body').toggleClass('mobile-menu-open')})
$(document).on('click','.mobile-menu-wrapper',function(e){$('body').removeClass('mobile-menu-open')})
...
if(!$headerLayout.hasClass('header_v4')){magicLine()}
...
if(background_gradient_2.length){$('.thim_overlay_gradient_2 rs-sbg-px > rs-sbg-wrap > rs-sbg').addClass('thim-overlayed')}},thim_single_image_popup:function(){if(jQuery().magnificPopup){$('.thim-single-image-popup').magnificPopup({type:'image',zoom:{enabled:true,duration:300,easing:'ease-in-out',}})}},check_load_menu:function(){var window_with_resize=$(window).width()
var type_menu_first=''
if(!flag_call_ajax_check_load_menu){return}
...
if(el_main_menu.length){type_menu_first='html_desktop'}else if(el_mobile_menu_inner.length){type_menu_first='html_mobile'}
console.log(type_menu_first)
flag_call_ajax_check_load_menu=0
setTimeout(function(){$.ajax({type:'post',url:ajaxurl,data:{action:'get_template_mainmenu_rezize',screen_size:$(window).width(),},beforeSend:function(){},success:function(response){console.log(response)
if(response.success){if(response.data['html_mobile']){$('.mobile-menu-wrapper').html(response.data['html_mobile'])}
if(response.data['html_desktop']){$('.width-navigation').html(response.data['html_desktop'])}}},complete:function(){flag_call_ajax_check_load_menu=1}})},300)}}
var flag_call_ajax_check_load_menu=1
$(document).ready(function(){thim_eduma.ready()
...
elementorFrontend.hooks.addAction('frontend/element_ready/thim-course-categories.default',thim_eduma.thim_carousel)
